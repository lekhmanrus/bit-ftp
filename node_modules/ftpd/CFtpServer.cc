...
bool CFtpServer::StopAccepting( void ) {
	bIsAccepting = false;
	OnServerEventCb( STOP_ACCEPTING );
	return bIsAccepting;
}
...
char* CFtpServer::CClientEntry::BuildPath( char* pszAskedPath, char **pszVirtualPath ) {
	if( !pUser )
...
bool CFtpServer::DeleteUser( CFtpServer::CUserEntry *pUser ) {
	if( pUser != NULL ) {
		...
		UserListLock.Enter();
		{
			...
			OnUserEventCb( DELETE_USER, pUser, NULL );
...
bool CFtpServer::CUserEntry::SetStartDirectory( const char *pszStartDir ) {
	char *pszSDEx = strdup( pszStartDir );
	if( !pszSDEx ) return false;
	SimplifyPath( pszSDEx );
	struct stat st; // Verify that the StartPath exist, and is a directory
	if( stat( pszSDEx, &st ) != 0 || !S_ISDIR( st.st_mode ) ) {
		free( pszSDEx );
	...
	strcpy( szStartDirectory, pszSDEx ); free( pszSDEx );
	return true;
}
...
CFtpServer::CClientEntry::~CClientEntry() {
	...
	if( pFtpServer ) {
		...
		if( pFtpServer->pFirstClient == this )
			pFtpServer->pFirstClient = pNextClient;
		if( pFtpServer->pLastClient == this )
			pFtpServer->pLastClient = pPrevClient;
	}
	ClientLock.Destroy();
}
		...
		if( nCmd == CMD_NONE ) {
  		...
  		continue;
				...
				&& ( iPort1 | iPort2 ) != 0 )
			{
				...
				if( ulPortIp == pClient->ulClientIP || pClient->ulClientIP == inet_addr( "127.0.0.1" ) ) {
			...
			if( nCmd == CMD_STAT && !pszCmdArg ) {
				pClient->SendReply( "211 CFtpServer / Browser FTP Server" );
				continue;
			}
