...
Handle<Value> GetOptions(const Arguments& args) {
  HandleScope scope;
  Local<Object> ret = Object::New();
  unsigned short int usStart;
  unsigned short int usLen;
  pFtpServer->GetDataPortRange(&usStart, &usLen);
  ret->Set(String::New("dataPortRangeFrom"), Uint32::New(usStart));
  ret->Set(String::New("dataPortRangeTo"), Uint32::New(usStart + usLen));
  ret->Set(String::New("noLoginTimeout"), Uint32::New(pFtpServer->GetNoLoginTimeout())); 
  ret->Set(String::New("noTransferTimeout"), Uint32::New(pFtpServer->GetNoTransferTimeout())); 
  ret->Set(String::New("checkPassDelay"), Uint32::New(pFtpServer->GetCheckPassDelay())); 
  ret->Set(String::New("maxPasswordTries"), Uint32::New(pFtpServer->GetMaxPasswordTries())); 
  ret->Set(String::New("listenPort"), Uint32::New(pFtpServer->GetListeningPort())); 
  return scope.Close(ret);
}

Handle<Value> SetOptions(const Arguments& args) {
  HandleScope scope;
  if(args.Length() < 7) {
    ThrowException(Exception::Error(String::New("Wrong number of arguments. ")));
    return scope.Close(Boolean::New(false));
  ...
  if(!args[2]->IsUndefined() && !args[2]->IsUint32()) {
    ThrowException(Exception::TypeError(String::New("Wrong arguments [3]. ")));
    return scope.Close(Boolean::New(false));
  }
  if(!args[3]->IsUndefined() && !args[3]->IsUint32()) {
    ThrowException(Exception::TypeError(String::New("Wrong arguments [4]. ")));
    return scope.Close(Boolean::New(false));
  }
  if(!args[4]->IsUndefined() && !args[4]->IsUint32()) {
    ThrowException(Exception::TypeError(String::New("Wrong arguments [5]. ")));
  ...
  if(!args[5]->IsUndefined() && !args[5]->IsUint32()) {
    ThrowException(Exception::TypeError(String::New("Wrong arguments [6]. ")));
    return scope.Close(Boolean::New(false));
  }
  if(!args[6]->IsUndefined() && !args[6]->IsUint32()) {
    ThrowException(Exception::TypeError(String::New("Wrong arguments [7]. ")));
    return scope.Close(Boolean::New(false));
  }
  unsigned int dataPortRangeFrom = args[0]->Int32Value(),
               dataPortRangeTo   = args[1]->Int32Value(),
               noLoginTimeout    = args[2]->Int32Value(),
               noTransferTimeout = args[3]->Int32Value(),
               checkPassDelay    = args[4]->Int32Value(),
               maxPasswordTries  = args[5]->Int32Value();
  listenPort = args[6]->Int32Value();
  if(!pFtpServer->SetDataPortRange(dataPortRangeFrom, dataPortRangeTo - dataPortRangeFrom)) {
    ThrowException(Exception::Error(String::New("Incorrect data port range. ")));
    return scope.Close(Boolean::New(false));
  }
  pFtpServer->SetNoLoginTimeout(noLoginTimeout);
  pFtpServer->SetNoTransferTimeout(noTransferTimeout);
  pFtpServer->SetCheckPassDelay(checkPassDelay);
  pFtpServer->SetMaxPasswordTries(maxPasswordTries);
  pFtpServer->SetListeningPort(listenPort);
  return scope.Close(Boolean::New(true));
}

Handle<Value> GetClientsOnlineCount(const Arguments& args) {
  HandleScope scope;
  if(!pFtpServer->IsListening())
    return scope.Close(Uint32::New(0));
  return scope.Close(Uint32::New(pFtpServer->GetNbClient()));
}

Handle<Value> GetUsersOnlineCount(const Arguments& args) {
  HandleScope scope;
  if(!pFtpServer->IsListening())
    return scope.Close(Uint32::New(0));
  return scope.Close(Uint32::New(pFtpServer->GetNbUser()));
}
...
void Init(Handle<Object> exports) {
  ...
  exports->Set(String::NewSymbol("getOptions"), FunctionTemplate::New(GetOptions)->GetFunction());
  exports->Set(String::NewSymbol("setOptions"), FunctionTemplate::New(SetOptions)->GetFunction());
  exports->Set(String::NewSymbol("getClientsOnlineCount"), FunctionTemplate::New(GetClientsOnlineCount)->GetFunction());
  exports->Set(String::NewSymbol("getUsersOnlineCount"), FunctionTemplate::New(GetUsersOnlineCount)->GetFunction());
